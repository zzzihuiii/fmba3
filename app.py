...
def index():
    if request.method == "POST":
        ...
        s1 = "The score of credit card upgrade based on decision tree is: " + str(pred1[0])
        
        model2 = joblib.load("CCU_GB")
        pred2 = model2.predict([[purchases, suppcard]])
        s2 = "The score of credit card upgrade based on SGB is: " + str(pred2[0])
        
        model3 = joblib.load("CCU_NN")
        pred3 = model3.predict([[purchases, suppcard]])
        s3 = "The score of credit card upgrade based on neural network is: " + str(pred3[0])
        
        model4 = joblib.load("CCU_Reg")
        pred4 = model4.predict([[purchases, suppcard]])
        s4 = "The score of credit card upgrade based on linear regression is: " + str(pred4[0])
        
        model5 = joblib.load("CCU_RF")
        pred5 = model5.predict([[purchases, suppcard]])
        s5 = "The score of credit card upgrade based on decision tree is: " + str(pred5[0])
